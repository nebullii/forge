...
{
  "api": [
    {"method": "GET", "path": "/health", "response_schema": {"ok": "bool"}, "auth": "none"}
  ],
  "models": []
}
```"""
    ...
    def test_backend_with_files_passes(self):
        r = validate_agent_output("backend", self.BACKEND_RESPONSE, [("app.py", "print(1)")])
        assert r.valid
    ...
    def test_empty_file_content_fails(self):
        r = validate_agent_output("backend", self.BACKEND_RESPONSE, [("app.py", "")])
        assert not r.valid
    ...
    def test_multiple_files_pass(self):
        ...
        r = validate_agent_output("backend", self.BACKEND_RESPONSE, files)
        assert r.valid
        assert r.files_found == 3

    def test_backend_without_contracts_fails(self):
        r = validate_agent_output("backend", "code", [("app.py", "print(1)")])
        assert not r.valid
        assert "machine-readable contracts" in r.reason
